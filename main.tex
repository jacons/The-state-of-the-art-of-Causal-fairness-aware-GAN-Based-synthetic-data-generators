...
The privacy limitations are not the only aspect; another essential challenge is related to the fairness of the data. 
...
In formula~\footnote{I have slightly change the notation presented in~\cite{DBLP:PearlCausality2009}, to be consistent with the whole report.}:
\begin{equation}
...
The ``fair'' generative models aim to approximate the distribution $P$ with $P'$ such that a synthetic dataset $\mathcal{D}' \sim P'$ is as close as possible to $\mathcal{D}$ and discrimination-free. 
This property ensures that ML models trained on that data are fair~\cite{DBLP:DECAF, DBLP:Xu_Depeng}. 
For the remainder of the paper, we use the following notation: we refer to the training set and real data distribution as $\mathcal{D}$ and $P$, respectively.
...
To keep the work self-contained for the rest of this section, we describe the principal notion of fairness. 
...
In summary, in the vanilla GAN, the generator is a single neural network; on the other hand, in a CausalGAN, the generator is a set of CGANs, one for each feature, which are connected by the causal relationship imposed by the graph $\mathcal{G}$.
...
The core idea is to extend the vanilla GAN architecture (Section~\ref{sec:gan}) by incorporating a fairness constraint into the loss function. 
...
The authors show that a generator trained under this principle produces synthetic data such that \textit{\textbf{any}} classifier trained on it (i.e., ${\hat{\bm{X}}, \hat{Y}}$) satisfies the Demographic Parity (DP) criterion on real datasets.

Formally, let us assume a dataset $\mathcal{D}$ (as defined in Section~\ref{sec:sdg_def}) where both target $\mathcal{Y}$ and the protected feature $\mathcal{A}$ are binary.
The architecture consists of one generator, $G$, and two discriminators, $D^1$ and $D^2$. 
The generator generates fake samples $\mathcal{D}' = \{(\bm{x}', y')\}$ conditioned on the protected attribute $a \sim P(A)$, $D^1$ aims to ensure that generated data $\{(\bm{x}', a, y')\}$ is close to the real data $\{(\bm{x}, a , y)\}$ as possible, while the other discriminator $D^2$ aims to ensure there are no correlation between $\bm{X}'$ and $A$ and no correlation between $Y'$ and $A$.
The architecture requires that the protected attribute $a$ represents an input for the generator, in addition to the noise. Such $a$ is sampled from the marginal distribution of the dataset $\mathcal{D}$.

In summary, FairGAN generates unprotected attributes $\bm{x}'$ and decision $y'$ given $a$, while ensuring $I(\hat{X}, \hat{Y}, A) = 0$.
Thus, the synthetic data satisfies Demographic Parity (Definition~\ref{def:dp}) and FTU (Definition~\ref{def:ftu}).

\newpage
Mathematically, FairGAN optimises:
\begin{equation}
...
The parameter $\lambda$ controls the trade-off between realism and fairness.
Figure~\ref{fig:fairgan} shows the model's architecture. 
Focusing on the second objective function in the Equation~\ref{eq:fairgan_loss}, $J_2$ incentivises the discriminator $D^2$ to try to distinguish the data distribution derived from the different protected value conditions (i.e., data in which $A=1$ from data where $A=0$) while the generator tries to fool it. 
In the trained model, we achieve the situation in which $P_G(\bm{X}',Y'|A=0) \approx P_G(\bm{X}', Y' | A=1)$. 
The principal limitation of FairGAN will be discussed in Section~\ref{sec:discussion}.
...
\label{sec:CFGAN}
Causal fairness-aware GAN (CFGAN)~\cite{DBLP:CFGAN} represents GAN--based SDG, which ensures fairness in the synthetic dataset by using the notion of fairness (see Section~\ref{sec:sdg_def}) integrated in the loss functions as in FairgAN.
However, in this architecture, causal reasoning is also exploited (Section~\ref{sec:scm}). 
In detail, the model consists of two generators, $G^1$ and $G^2$, and two discriminators, $D^1$ and $D^2$. 
One discriminator is used to optimise the data utility, and another to enforce the fairness constraint.
The generators, instead, follow the implementation of CausalGAN (Section~\ref{sec:causal_gan}), which is composed of a set of sub-generators, in formula, $G^i = \{G_0^i,\dots, G_d^i\}$ for $i = \{1,2\}$.
One of the most important aspects is that the architecture assumes a dataset $\mathcal{D}$ and a causal graph $\mathcal{G}$ that faithfully respects the dataset.
...
To introduce the algorithm, we need to describe some criteria of fairness: total effect, path-specific effect, and counterfactual effect.
...
\begin{definition}[Path-specific effect]
    ...
    where $P(Y_{\bm{x}}|\pi)$ represents the interventional distribution where the intervention is transferred only along $\pi$. When $SE_\pi(\bm{x}_2, \bm{x}_1)=0$, it means no specific effect.
\end{definition}    
%
...
On the other hand, in $G^2$, an intervention of $a$ is applied, in formula $P(Y|\mathrm{do}(A=a))$, and the aim is to minimise the Total effect.
We identify $P(Y_{a+})$ when $a=1$ and $P(Y_{a-})$ when $a=0$.
Concerning the discriminators, $D^1$ is designed to distinguish between the real data $(x, y, a) \sim P(\bm{X}, Y, A)$ and the generated data $(\bm{x}', y', a') \sim P_{G^1}(\bm{X}', Y', A')$. 
$D^2$ is employed to distinguish between the two interventional distributions: $y'_{a+} \sim P_{G^2}(Y'_{a+})$ and $y'_{a-} \sim P_{G^2}(Y'_{a-})$.
Thus, the overall cost function is defined as in Equation~\ref{eq:overall_loss} where:

\begin{equation}
    \begin{split}
        J_1(G^1, D^1) &= \mathbb{E}_{(\bm{x}, y, a) \sim P(\bm{X}, Y, A)}[\log D^1(\bm{x}, y, a)] + \mathbb{E}_{(\bm{x}', y', a') \sim P_{G^1}(\bm{X}', Y', A')}[1 - \log D^1(\bm{x}', y', a')]
        \\
        J_2(G^2, D^2) &= \mathbb{E}_{y'_{a^+} \sim P_{G^2}(Y'_{a^+})}[\log D^2(y'_{a^+})] + \mathbb{E}_{y'_{a^-} \sim P_{G^2}(Y'_{a^-})}[1 - \log D^2(y'_{a^-})]
    \end{split}
    \label{eq:CFGAN_te}
\end{equation}

Optimising Equation~\ref{eq:CFGAN_te} means obtaining a situation in which $P(Y'_{a+}) \sim P(Y'_{a-})$ that corresponds to no total effect change (Definition~\ref{def:te}). 
In other words, we trained a GAN to generate the same data distribution independently of the protected attribute.
...
In the reference setting $G^2_s = 0$ always, for interventional setting $G^2_a = 1$ if $a = a^+$ and $G^2_a = 0$ if $a = a^-$.
...
%
\begin{equation}
    J_2(G^2, D^2) = \mathbb{E}_{y'_{a^+|\pi} \sim P_{G^2}(Y'_{a^+|\pi})}[\log D^2(y'_{a^+|\pi})] + \mathbb{E}_{y'_{a^-|\pi} \sim P_{G^2}(Y'_{a^-|\pi})}[1 - \log D^2(y'_{a^-|\pi})]
    \label{eq:CFGAN_se}
\end{equation}
...
\begin{figure}
    ...
    \caption{An example of the generator $G^2$ for CFGAN based on indirect discrimination. $S$ (\textit{the sensible attribute that we call A}) is set to $1$ or $0$ and the transmission is set only along $\pi$ = $\{S \to B \to Y \}$ to sample from the interventional distributions $P_{G^2}(A_{s^+}|\pi, B_{s+}|\pi , Y_{s+}|\pi)$ (red) and $P_{G^2} (A_{s^-}|\pi , B_{s^-}|\pi , Y_{s^-}|\pi )$ (green) respectively. $S$ is set to be $0$ for the reference setting.}
...
Previously, we conditioned only on $A$ (Total Effect and Indirect Discrimination); now we condition also on the subset features $\bm{O}$ (e.g., $\bm{O} = \{race, native\_country\}$).
...
Finally, we utilise $G^2$, with the noise vector $\bm{z}$, to generate samples from the interventional distribution on $A$ and $\bm{O}$ denoted by $P_{G^2}(\bm{X}'_a,Y'_a|\bm{o})$. 
The discriminator $D^2$ is revised to distinguish from $y'_{a^+|o} \sim P(Y'_{a+|o})$ and $y'_{a^-|o} \sim P(Y'_{a-|o})$.
Hence, $J_2$ loss function becomes:

\begin{equation}
    J_2(G^2, D^2) = \mathbb{E}_{y'_{a^+|\bm{o}} \sim P_{G^2}(Y'_{a^+|\bm{o}})}[\log D^2(y'_{a^+|\bm{o}})] + \mathbb{E}_{y'_{a^-|\bm{o}} \sim P_{G^2}(Y'_{a^-|\bm{o}})}[1 - \log D^2(y'_{a^-|\bm{o}})]
    \label{eq:CFGAN_ce}
\end{equation}

\subsection{DECAF}
\label{sec:DECAF}
DEbiasing CAusal Fairness (DECAF)~\cite{DBLP:DECAF} denotes an approach for synthetic dataset generation that assumes a dataset $\mathcal{D}$ and a given causal graph $\mathcal{G}$.
DECAF is a causal generative model based on structural equation models that implements these mechanisms through conditional GANs as done for CausalGAN (Section~\ref{sec:causal_gan}).
The key difference with other approaches in the trained model is the capacity to intervene in the derived SCM before generating the data, thereby removing dependencies that lead to unfairness in a downstream model (these can be specific to the fairness metric considered). 
...
The architecture, distinct from CFCGAN and DECAF, consists of two generators, each implemented as a vanilla GAN, and two discriminators. 
CFSDG does not require a causal graph $\mathcal{G}$.
However, the model requires a dataset $\mathcal{D}$ that comprises $\bm{X}$ (unprotected features), $A$ (a binary protected feature), $Y$ (the target), and finally, to define a random variable $U$ as noise. 
Figure~\ref{fig:CFSDG} depicts the CFSDG architecture.
...
\begin{equation}
    \begin{split}
        ...
        J_2 &= \lambda \mathbb{E}_{u \sim P(U),a \sim P(A)}[ (G_Y(u,a) - G_Y(u,\neg a))^2]
    \end{split}
    \label{eq:cfsdg_loss}
\end{equation}
...
Table \ref{tab:comparison} shows the main difference among the generative models.
...
Thus, the author built an architecture that guarantees that \textit{any} ML classifier trained on the synthetic data is fair, imposing $I(\hat{\bm{X}}, \hat{Y}, A) = 0$. 
However, subsequent work (Sections~\ref{sec:CFGAN}, ~\ref{sec:DECAF}, ~\ref{sec:CFSDG}) criticises FairGAN for imposing overly strict independence constraints and for architectural limitations.
Specifically, they suggest relaxing FairGAN’s constraint: instead of requiring $I(\hat{\bm{X}}, \hat{Y}, A)=0$, it is sufficient to enforce $I(\hat{Y}, A)=0$ to achieve Demographic Parity~\cite{counterfactual}.
Such strong restrictions reduce the utility of the generated data compared to alternative architectures.
CFGAN (Section~\ref{sec:CFGAN}) w.r.t FairGAN introduced the notion of causal fairness and more advanced fairness criteria (e.g. total effect, path effect and counterfactual effect). Moreover, experimental results show that CFGAN  w.r.t. FairGAN perform worse in Total effect (Definition~\ref{def:te}) but better in Path-specific effect (Definition~\ref{def:se})~\cite{DBLP:CFGAN}[Table 1].

Nevertheless, methods such as CFGAN and DECAF (Section~\ref{sec:DECAF}), which empirically demonstrate that they outperform FairGAN, require a strong assumption: the availability of a causal graph that is faithful w.r.t. the data distribution in which the trained dataset was derived. 
...
The DECAF's goal is not limited to achieving statistical constraints in the data, but also claims that the predictors trained on such data are fair in real datasets as well.
...
Now, for satisfying DP (Definition~\ref{cor:dp_debias}), we need to remove the edge from $X$ to $Y$, then $Y$ will be either constant or a distribution independent from $X$ and so $m$.
...
The main difference with respect to the rest of the papers lies in the intended effect of the generated data on machine learning models trained on it. 
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        \rowcolor[HTML]{C6D8FF} 
        \textbf{Framework} & \textbf{FairGAN}~\cite{DBLP:Xu_Depeng} & \textbf{CFGAN}~\cite{DBLP:CFGAN} & \textbf{DECAF}~\cite{DBLP:DECAF} & \textbf{CFSDG}~\cite{counterfactual} \\ \hline
        \textbf{Allow multivalued for $A$}  & \ding{55} & \ding{55} & \ding{51} & \ding{55} \\ \hline
        \rowcolor[HTML]{EFEFEF} 
        \textbf{Causal Notion of f.}        & \ding{55} & \ding{51} & \ding{51} & \ding{51} \\ \hline
        \textbf{Require $\mathcal{G}$}      & \ding{55} & \ding{51} & \ding{51} & \ding{55} \\ \hline
        \rowcolor[HTML]{EFEFEF} 
        \textbf{Interventional f.}          & \ding{55} & \ding{51} & \ding{55} & \ding{55} \\ \hline
        \textbf{Fairness in $\mathcal{L}$}  & \ding{51} & \ding{51} & \ding{55} & \ding{51} \\ \hline
        \rowcolor[HTML]{EFEFEF} 
        \textbf{Counterfactual f.}          & \ding{55} & \ding{55} & \ding{55} & \ding{51} \\ \hline
    
    \end{tabular}
    \label{tab:comparison}
    \caption{The table shows the main difference among the algorithms.}
\end{table}
