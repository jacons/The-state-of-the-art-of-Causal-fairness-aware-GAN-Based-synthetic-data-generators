...
\label{sec:CFGAN}
Causal fairness-aware GAN (CFGAN)~\cite{DBLP:CFGAN} represents GAN--based SDG, which ensures fairness in the synthetic dataset by using the fairness notion (see Section~\ref{sec:fairness_notions}) in the loss function, similarly to FairGAN (Section~\ref{sec:fairgan}), but also exploits causal reasoning (Section~\ref{sec:scm}). 
The architecture assumes a dataset $\mathcal{D}$ and a causal graph $\mathcal{G}$ that faithfully respects the dataset.
CFGAN relies on the CausalGAN (Section~\ref{sec:causal_gan}) but extends the architecture by including an additional generator and discriminator.
...
To achieve no indirect discrimination, the loss change as follows:    
%
\begin{equation}
...
FairGAN (Section~\ref{sec:fairgan}) aim to create generative models which produce datasets that satisfy the Demographic Parity (Definition~\ref{def:dp}). 
...
With CFGAN, the idea was born to introduce the \emph{do-operator} in order to minimise the difference in distribution among the protected values using the interventions and the counterfactuals (see Section~\ref{sec:scm}). 
...
The DECAF's goal is not limited to achieving statistical constraint in the data, but also claims that the predictors trained on such data are fair too \footnote{When we discuss CFSDG, we will discover that this is not theoretically grounded.}.
...
Now, for satisfying DP (Definition~\ref{cor:dp_debias}), we need to remove both edges from $A$ to $X$ and then from $X$ to $Y$, then $Y$ will be either constant or a distribution independent from $X$ and so $m$. Increasing $m$ we can have a large KL-distance between $P$ and $P'$.
...
DECAF and FairGAN's goals are much stronger; they aim to create a dataset such that every predictor trained on it will satisfy DP.
...
Starting from FairGAN, which introduced fairness constraints to address DP, we examined further approaches such as CFGAN, DECAF, and CFSDG, which incorporate causal models and counterfactual reasoning to provide stronger fairness guarantees.
While early models like FairGAN primarily aimed to address the DP constraint, recent methods, such as CFGAN, introduced interventions and counterfactuals into the generative process. 
