...
\label{sec:CFGAN}
Causal fairness-aware GAN (CFGAN)~\cite{DBLP:CFGAN} represents GAN--based SDG, which ensures fairness in the synthetic dataset by using the fairness notion (see Section~\ref{sec:sdg_def}) integrated in the loss functions.
...
Concerning the generators, they are composed of a set of sub-generators, in formula, $G^i = \{G_0^i,\dots, G_d^i\}$ for $i = \{1,2\}$.
